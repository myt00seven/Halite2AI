...
game = hlt.Game("NR7-Attacker-v19")
...
while True:
    ...
    logging.info("List of enemy ships: " + str([i.id for i in enemyShips]))
    ...
    for task in task_priority:
            ...
            if task == "combat":
                if not ship.command and len(closest_enemies[ship.id])>0 and closest_enemies[ship.id][0][1] < COMBAT_RANGE:
                    if ship.field_score(closest_allies, closest_enemies) <= -0.75:
                        ...
                        ship.issueCommand(ship.navigate(ship.keep_distance_from(enemyship, closest_enemies, closest_allies),game_map), "combat-retreat", enemyship.id)
            ...
            if task =="resume_defense" and ship.assigned_task:
                if not ship.command and ("defense" in ship.assigned_task) and (ship.assigned_target_id in enemyShips):
                    ...
                    if target_ship and target_ship.health>0:
                        ...
                        ship.issueCommand(ship.navigate(ship.closest_point_to(target_ship.predict_next_pos(), grouping = True, closest_allies = closest_allies),game_map), "re-defense", target_ship.id)
        ...
        # logging.info("Time consumed by command:\t"+ task +'\t'+ str(time.time() - startTime))
