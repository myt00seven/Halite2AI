...
game = hlt.Game("NR7-Attacker-v25")
        ...
        for ship in AllFreeShips:
            ...
            if task == "combat":
                if not ship.command and len(closest_enemies[ship.id])>0 and closest_enemies[ship.id][0][1] < COMBAT_RANGE:
                    if ship.field_score(closest_allies, closest_enemies, cnp) <= -0.75:
                        # retreat 
                        enemyship = closest_enemies[ship.id][0][0]
                        ship.issueCommand(ship.navigate(ship.keep_distance_from_ori(enemyship, game_map, closest_enemies, closest_allies, closest_all_instances, number_of_players = cnp, group_ally = True),game_map), "combat-retreat", enemyship.id)
                    elif -0.75< ship.field_score(closest_allies, closest_enemies, cnp) < 0:
                        enemyship = closest_enemies[ship.id][0][0]
