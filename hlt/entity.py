...
class Entity:
    ...
    def __init__(self, x, y, radius, health, player, entity_id):
        ...
        self.owner = player
        self.id = entity_id
        self.density_score = 1
        self.far_from_center_score = 1
    ...
    def calculate_distance_list(self, targetList):
        return np.array([self.calculate_distance_between(i) for i in targetList])
    ...
    def keep_distance_from_ori(self, target, game_map, closest_enemies, closest_allies, closest_all_instances, number_of_players = 2, group_ally = True):

        vec = Position(target.x, target.y)
        count = 1
        for (enemyship, dist) in closest_enemies[self.id]:
            if dist > constants.COMBAT_RANGE:
                break
            vec.x += enemyship.x
            vec.y += enemyship.y
            count += 1
        vec.x /= count
        vec.y /= count
        angle = self.calculate_angle_between(Position(vec.x, vec.y))
        # pos_predict = Position(target.x, target.y)
        # pos_predict = target.predict_next_pos()
        # if self.calculate_distance_between(pos_predict) > self.calculate_distance_between(target):
            # pos_predict.x ,  pos_predict.y = target.x , target.y
        # min_distance=3        
        # radius = constants.WEAPON_RADIUS + min_distance
        radius = constants.DEFAULT_SPEED
        x = self.x - radius * math.cos(math.radians(angle))
        y = self.y - radius * math.sin(math.radians(angle))
        escape_point = Position(x,y)

        vec = Position(0, 0)
        count = 0
        for (allyship, dist) in closest_allies[self.id]:
            if dist > constants.RESCUE_RANGE:
                break
            if allyship.assigned_task == "rescue":
                vec.x += allyship.predict_next_pos().x
                vec.y += allyship.predict_next_pos().y
                count += 1
        if count>0:
            vec.x /= count
            vec.y /= count
            support_point = Position(vec.x,vec.y)
        else:
            support_point = escape_point

        cRETREAT_ALLY_SPOT_RATIO = 0.5
        newx = (escape_point.x * (1-cRETREAT_ALLY_SPOT_RATIO)+support_point.x * cRETREAT_ALLY_SPOT_RATIO)
        newy = (escape_point.y * (1-cRETREAT_ALLY_SPOT_RATIO)+support_point.y * cRETREAT_ALLY_SPOT_RATIO)
        
        return Position(newx,newy)    

    def keep_distance_from(self, target, game_map, number_of_players = 2, group_ally= True):

        escape_point = self.closest_point_to(target, min_distance = constants.RADIUS_ESCAPE)
        closest_allies = game_map.distances_dict_ally
        ...
        if group_ally:
            cRETREAT_ALLY_SPOT_RATIO = constants.getRETREAT_ALLY_SPOT_RATIO(number_of_players)
            newx = (escape_point.x * (1-cRETREAT_ALLY_SPOT_RATIO)+support_point.x * cRETREAT_ALLY_SPOT_RATIO)
            newy = (escape_point.y * (1-cRETREAT_ALLY_SPOT_RATIO)+support_point.y * cRETREAT_ALLY_SPOT_RATIO)
        else:
            newx, newy = escape_point.x, escape_point.y
    ...
    def closest_point_to(self, target, min_distance=3, grouping = False, closest_allies = None, number_of_players = 2):
        """
        ...
        count = 0
        if grouping:
            for (allyship, dist) in closest_allies[thisid]:
                if dist > constants.getGROUPING_DISTANCE(number_of_players):
                    break
                if "defense" in allyship.assigned_task:
                    pos_predict = allyship.predict_next_pos()
                    posx += pos_predict.x
                    posy += pos_predict.y
                    count += 1
            if count >0:
                posx /= count
                posy /= count
                cGROUPING_NAVI_FACTOR = constants.getGROUPING_NAVI_FACTOR(number_of_players)
                x = (1-cGROUPING_NAVI_FACTOR)*x + cGROUPING_NAVI_FACTOR*posx
                y = (1-cGROUPING_NAVI_FACTOR)*y + cGROUPING_NAVI_FACTOR*posy
...
class Planet(Entity):
    ...
    def __init__(self, planet_id, x, y, hp, radius, docking_spots, current,
        ...
        self._docked_ships = {}
        self.incoming_ships = set()

        self.density_score = 0
        self.far_from_center_score = 0
...
class Ship(Entity):
    ...
    def __init__(self, player_id, ship_id, x, y, hp, vel_x, vel_y,
        ...
        self.targeted = set() # For enemy ship, record which of our ship is aiming at this ship
        self.command = None

        # past information storage
        ...
        self.dummyAttacker1 = None
        self.dummyAttacker2 = None

        # battle field score
        self.battle_field_score = -999
        self.assisted = 0

    def field_score(self, closest_allies, closest_enemies, num_of_players = 2):
        if self.battle_field_score != -999:
            return self.battle_field_score
        self.battle_field_score = self.health / constants.MAX_SHIP_HEALTH

        for (enemyship, dist) in closest_enemies[self.id]:
            if dist > constants.getSHORT_ASSESS_RANGE(num_of_players):
                break
            if enemyship.docking_status == Ship.DockingStatus.UNDOCKED:
                self.battle_field_score -= enemyship.health / constants.MAX_SHIP_HEALTH
            else:
                self.battle_field_score -= 0.3 * enemyship.health / constants.MAX_SHIP_HEALTH

        for (allyship, dist) in closest_allies[self.id]:
            if dist > constants.getSHORT_ASSESS_RANGE(num_of_players):
                break
            if allyship.docking_status == Ship.DockingStatus.UNDOCKED:
                self.battle_field_score += allyship.health / constants.MAX_SHIP_HEALTH
            else:
                self.battle_field_score += 0.3 * allyship.health / constants.MAX_SHIP_HEALTH

        return self.battle_field_score


    def predict_next_pos(self):
        return Position(self.x+(self.x-self.lastx), self.y+(self.y-self.lasty))

    def issueCommand(self, command, task, target_id):
        if command:
            self.command = command
            self.current_task = task
            self.current_target_id = target_id
            if constants.PRINT_ACTION_LOG: 
                logging.info("Action of Ship:"+str(self.id)+"\t"+task+"\ttarget: "+str(target_id))
        else:
            logging.info("Warning: Failed to generate command for task:"+task+" on target: "+ str(target_id))
    ...
    def thrust(self, magnitude, angle):
        ...
        rad = math.radians(round(angle))
        self.vx = math.cos(rad) * int(magnitude)
        self.vy = math.sin(rad) * int(magnitude)

        return "t {} {} {}".format(self.id, int(magnitude), round(angle))

    def clearThrust(self):
        self.vx = 0
        self.vy = 0
    ...
    # def navigateForDistractor

    def navigate(self, target, game_map, speedMax = constants.DEFAULT_SPEED, 
        avoid_obstacles=True,
                 ignore_ships=False, ignore_planets=False, max_corrections=60, avoid_enemy = False):

        # try to implement a two side search.
        ...
        for correction in range(max_corrections+1):
            for sign in [-1, 1]:
                thisAngle = round(angle + angular_step*correction*sign) % 360
                rad = math.radians(thisAngle)
                new_target_dx = math.cos(rad) * distance
                new_target_dy = math.sin(rad) * distance
                new_target = Position(self.x + new_target_dx, self.y + new_target_dy)

                speed = speedMax if (distance >= speedMax) else distance

                cmd = self.thrust(speed, thisAngle)

                if avoid_enemy:
                    if avoid_obstacles and game_map.obstacles_between_forDistractor(self, new_target):
                        continue
                else:
                    if avoid_obstacles and game_map.obstacles_between(self, new_target, ignore):
                        continue
    ...
    def navigate_ori(self, target, game_map, speed, avoid_obstacles=True, max_corrections=90, angular_step=1,
