...
task_priority = [
"resume_steal", # continue the steal task
...
"steal_weak_enemy", # docked enemy or impaired enemy in middle range
...
game = hlt.Game("NR7-Attacker-v30")
...
while True:
    ...
    closest_all_instances = game_map.get_all_instances_by_distance()
    ...
    # if PRINT_ACTION_LOG: logging.info("Time consumed before task list:\t"+ str(time.time() - startTime))
        ...
        for ship in AllFreeShips:
            ...
            if task == "combat":
                if not ship.command and len(closest_enemies[ship.id])>0 and closest_enemies[ship.id][0][1] < COMBAT_RANGE:
                    ...
                    if enemyship.battle_field_score - ship.battle_field_score > constants.COMBAT_THRESHOLD:
                        ship.issueCommand(ship.navigate(ship.keep_distance_from(enemyship, game_map, number_of_players = cnp, group_ally= True),game_map), "combat-retreat", enemyship.id)
                    else:
            ...
            elif task == "resume_steal" and ship.assigned_task and ("steal" in ship.assigned_task) and ALLOW_STEAL:
                if (ship.assigned_target_id in enemyShips):
                ...
                if target_ship:
                    ...
                    game_map.steal_team.append(ship)
                    ship.issueCommand(ship.navigate(ship.closest_point_to(target_ship),game_map, max_corrections=120, avoid_enemy = True), task, target_ship.id)


            elif task =="resume_defense" and ship.assigned_task:
                if not ship.command and ("defense" in ship.assigned_task) and (ship.assigned_target_id in enemyShips):
                    ...
                    if target_ship and target_ship.health>0:
                        target_ship.targeted.add(ship)
                        if enemyship.battle_field_score - ship.battle_field_score > constants.COMBAT_THRESHOLD:
                            ship.issueCommand(ship.navigate(ship.closest_point_to(target_ship, min_distance = constants.RADIUS_ESCAPE, grouping = True, closest_allies = closest_allies, number_of_players = cnp),game_map), "re-defense-wait-ally", target_ship.id)
                        else:
            ...
            elif task =="resume_conquer" and ship.assigned_task and ALLOW_DOCK:
            ...
            elif task == "rescue" and constants.ALLOW_RESCUE:
                if not ship.command:
                    ally_ship = if_rescuable_ally_ship(ship, closest_allies, closest_enemies, cnp)
                    if ally_ship:
            ...
            elif task == "steal_weak_enemy" and ALLOW_STEAL:
                if not ship.command and len(game_map.steal_team) < game_map.n_steal_allow:
                    target_ship = attackable_docking_ship(game_map, ship)
                    if target_ship:
                        target_ship.targeted.add(ship)
                        game_map.steal_team.append(ship)
            ...
            if (time.time()-startTime > constants.TimeThreshold):
                break

        # if PRINT_ACTION_LOG: logging.info("Time consumed by command:\t"+ task +'\t'+ str(time.time() - ctime))
